...
namespace MonteCarloOptionPricer.Models
{
    public class EuropeanOption : IOption
    {
            ...
            int vdcPoints = 1024)
        {
            ...
            // Initializing avgPayoff. Quck dirty fix to change later
            List<double> payoffs;
            double avgPayoff;
            int seed;

            if (p.SimMode == SimulationMode.ControlVariate)
            {
                seed = new Random().Next();
                RandomNumberGenerator.Seed(seed);
                var (terminalPrices, pnlHedges) = MonteCarloSimulator.SimulateControlVariateTerminals(p);
            ...
            else
            {
                ...
                payoffs = BuildPayoffs(terminals, p);
                avgPayoff = payoffs.Average();
